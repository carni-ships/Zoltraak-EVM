...
public final class EVMGPUBatchProver {
    ...
    private let gpuProver: ZoltraakCustomProver
    private var brakedownEngine: ZoltraakBrakedownEngine?
    private var ldeOptimizer: EVMLDEOptimizer?
    private var gpuMerkleEngine: EVMGPUMerkleEngine?
    private var leafHashEngine: ZoltraakLeafHashEngine?
    ...
    public init(config: Config = .standard) throws {
        ...
        self.gpuProver = ZoltraakCustomProver(config: .init(logBlowup: config.logBlowup, numQueries: config.numQueries))
        ...
        if config.commitmentStrategy == .brakedown || config.commitmentStrategy == .brakedownInterleaved {
            self.brakedownEngine = try ZoltraakBrakedownEngine()
        }
